...
- `html/template` for rendering, which escapes values by default; the template
  (`templates/page.html`), the stylesheet (`static/style.css`) and the schema (`schema.sql`)
  are compiled into the binary with `//go:embed`
